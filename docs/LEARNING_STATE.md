...
- Current task: create the first Git commit that records the environment baseline, learning handoff documents, and revised curriculum.
- Next curriculum task after the first commit: continue Week 0. The dependency-manager choice is deferred to Week 2, after the student has learned the relevant concepts from the textbook.
...
- Created a Git repository in this directory. It currently has no commits and uses the `master` branch.
